...
mod test {
    ...
    fn render_ui(app: &mut TreetopApp) -> String {
        ...
        for y in 0..area.height {
            for x in 0..area.width {
                ...
                } else if buffer[(x, y)].modifier.contains(Modifier::BOLD) {
                    crate::utils::test::bold(symbol)
                } else {
                    symbol.to_string()
                };
    ...
    fn does_not_filter_out_itself_when_asked_to() -> R<()> {
        ...
        set_pattern(&mut app, "bar")?;
        app.tick();
        assert!(render_ui(&mut app).contains(&crate::utils::test::bold("bar")));
        Ok(())
    }
